...
void ScreenWrap::Update([[maybe_unused]] double dt)
{
    ...
    {
        object.SetPosition(Math::vec2(object.GetPosition().x, 0 - frame_size.y / 2.0));
    }
    else if (object.GetPosition().y < 0 - frame_size.y / 2.0)
    {
        object.SetPosition(Math::vec2(object.GetPosition().x, Engine::GetWindow().GetSize().y + frame_size.y / 2.0));
    }
    if (object.GetPosition().x > Engine::GetWindow().GetSize().x + object.GetGOComponent<CS230::Sprite>()->GetFrameSize().x / 2.0)
    {
        object.SetPosition(Math::vec2(0 - frame_size.x / 2.0, object.GetPosition().y));
    }
    else if (object.GetPosition().x < 0 - frame_size.x / 2.0)
    {
        object.SetPosition(Math::vec2(Engine::GetWindow().GetSize().x + frame_size.x / 2.0, object.GetPosition().y));
    }
}
