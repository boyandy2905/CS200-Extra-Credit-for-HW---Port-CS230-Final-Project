...
public:
    ...
    std::unique_ptr<CS230::ShowCollision> global_show_collision{};
};
...
CS230::ShowCollision* Engine::GetGlobalShowCollision()
{
    return Instance().impl->global_show_collision.get();
}

void Engine::AddFont(const std::string& path)
{
    Instance().impl->fonts.emplace_back(std::make_unique<CS230::Font>(std::filesystem::path(path)));
}

CS230::Font& Engine::GetFont(std::size_t index)
{
    return *Instance().impl->fonts.at(index);
}
...
void Engine::Update()
{
    ...
    impl->window.Update();
    impl->input.Update();
    // Update global show-collision before state updates so it can respond to Tab anywhere
    if (impl->global_show_collision)
        impl->global_show_collision->Update(Engine::GetWindowEnvironment().DeltaTime);
    ...
    // Begin a renderer scene for this frame so that the ImmediateRenderer2D
    // has a proper view-projection matrix (NDC) to use when drawing textures
    // and primitives from game states.
    impl->renderer2D.BeginScene(CS200::build_ndc_matrix(viewport_size));

    state_manager.Draw();

    // End the renderer scene to flush any batched draw calls for this frame.
    impl->renderer2D.EndScene();
    if (!impl->window.IsHeadless())
    {
    ...
    {
        // In headless mode, keep viewport unchanged
        impl->viewport = impl->viewport;
    }
}

void Engine::CreateGlobalShowCollision()
{
    if (!Instance().impl->global_show_collision)
    {
        Instance().impl->global_show_collision = std::make_unique<CS230::ShowCollision>();
    }
}
