...
{
    Input::Input()
    : keys_down(static_cast<size_t>(Keys::Count), false), previous_keys_down(static_cast<size_t>(Keys::Count), false)
    {}
    ...
    void Input::Update()
    {
        ...
        {
            SDL_Scancode sdl_key = convert_cs230_to_sdl(static_cast<Keys>(i));
            keys_down[i] = keyboard_state[sdl_key] != 0;
        }
    }

    void Input::SetKeyDown(Keys key, bool is_pressed)
    {
        keys_down[static_cast<std::size_t>(key)] = is_pressed;
    }
    ...
    SDL_Scancode Input::convert_cs230_to_sdl(CS230::Input::Keys cs230_key)
    {
        switch (cs230_key)
        {
            ...
            case CS230::Input::Keys::LeftShift: return SDL_SCANCODE_LSHIFT;
