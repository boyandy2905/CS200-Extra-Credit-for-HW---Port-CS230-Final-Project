...
namespace CS200
{
    ...
    inline Math::vec2 to_ndc(const Math::vec2& screen_coord, Math::ivec2 view_size) noexcept
    {
        return {
            (2.0 * screen_coord.x) / static_cast<double>(view_size.x) - 1.0,
            (2.0 * screen_coord.y) / static_cast<double>(view_size.y) - 1.0
        };
    }

    inline Math::vec2 to_screen(const Math::vec2& ndc_coord, Math::ivec2 view_size) noexcept
    {
        return {
            (ndc_coord.x + 1.0) * 0.5 * static_cast<double>(view_size.x),
            (ndc_coord.y + 1.0) * 0.5 * static_cast<double>(view_size.y)
        };
    }

    inline bool to_offscreen(const Math::vec2& screen_coord, Math::ivec2 view_size) noexcept
    {
        return screen_coord.x < 0.0 || screen_coord.x > view_size.x ||
               screen_coord.y < 0.0 || screen_coord.y > view_size.y;
    }
}
