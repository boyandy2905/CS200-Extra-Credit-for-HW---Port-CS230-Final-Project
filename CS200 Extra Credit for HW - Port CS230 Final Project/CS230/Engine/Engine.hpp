...
public:
    ...
    // Access the global ShowCollision component (if created via CreateGlobalShowCollision)
    static CS230::ShowCollision* GetGlobalShowCollision();
    // Create a global ShowCollision component that listens to input regardless of state
    void CreateGlobalShowCollision();
    /**
     * \brief Add a bitmap font to the engine's font list
     * \param path Path to the font texture file
     */
    static void AddFont(const std::string& path);

    /**
     * \brief Get a loaded Font by index
     * \param index Index of previously added font
     * \return Reference to Font object
     */
    static CS230::Font& GetFont(std::size_t index);
